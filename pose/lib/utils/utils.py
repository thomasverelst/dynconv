...
def create_logger(cfg, cfg_name, phase='train', dry=False):
    ...
    log_file = 'running.log' if not dry else 'test.log'
...
def save_checkpoint(states, is_best, output_dir,
                    filename='checkpoint.pth',epoch=None):
    torch.save(states, os.path.join(output_dir, str(filename)))
    try:
        if epoch is not None and epoch % 5 == 0:
            torch.save(states, os.path.join(output_dir, 'checkpoint_'+str(epoch)+'.pth'))
    except:
        print('could not extra checkpoint')

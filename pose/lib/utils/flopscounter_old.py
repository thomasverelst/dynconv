...
def relu_flops_counter_hook(module, input, output):
    ...
    if module.__mask__ is not None:
        active_elements_count *= torch.sum(module.__mask__.hard).item() / module.__mask__.hard.numel()
...
def bn_flops_counter_hook(module, input, output):
    ...
    if module.affine:
        batch_flops *= 2
    if module.__mask__ is not None:
        batch_flops *= torch.sum(module.__mask__.hard).item() / module.__mask__.hard.numel()
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    if conv_module.__mask__ is not None:
        # (b, 1, h, w)
        flops_mask = conv_module.__mask__.hard.expand(batch_size, 1, output_height, output_width).float()
        active_elements_count = flops_mask.sum().item()
